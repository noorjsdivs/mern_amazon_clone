...
const CartItem = ({ item }: Props) => {
  const discount = (item?.price / 100) * item?.discountPercentage;
  const actualPrice = discount + item?.price;
  ...
  return (
    <div className="p-2 lg:p-4 flex gap-1 justify-between items-center bg-white rounded">
      ...
      <div className="w-2/5">
        ...
        <div className=" mt-2 rounded-lg font-semibold text-green-600">
          In Stock : {item?.stock}
        </div>
      </div>
      ...
      <div>
        ...
        <PriceFormate className="font-bold" price={item?.price} />
      </div>
      <div>
        <button onClick={() => handleDelete(item?.id)}>
          <RiDeleteBinLine
            size={24}
            className="hover:text-darkYellow duration-300"
          />
        </button>
      </div>
    </div>
